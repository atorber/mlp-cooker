...
import { getIcafeLastweek } from '@/services/aihc-mentor/api';

const { Title, Text } = Typography;

const IcafeLastweek: React.FC = () => {
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      const response = await getIcafeLastweek();
      setResult(response);
      if (response.success) {
        message.success('上周问题统计报告生成成功！');
      } else {
  ...
  return (
    <PageContainer
      title="上周问题统计"
      subTitle="统计和分析上周的客户问题数据，生成详细的问题报告"
    >
      <ProCard>
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          ...
          />
          <Title level={3}>问题统计分析</Title>
          <Text type="secondary">
            自动统计上周的客户问题数据，包括问题类型、状态分布、处理情况等详细信息
          </Text>
        </div>
