...
![架构](./docs/images/WX20251121-203945.png)
...
- 💾 **数据集管理**：数据集的上传、查询、版本管理
- 🗄️ **存储管理**：统一管理对象存储（BOS）和并行文件系统（PFS），实现基础资源的灵活调配
- 🏦 **数据仓库**：集成 lakeFS，支持类似 Git 的数据版本控制与多分支版本管理
- 🤖 **模型管理**：模型的创建、查询、版本管理
- 💻 **开发机管理**：云端开发实例的创建、控制台访问（Web Terminal）、生命周期管理（启停及回收）
- 🐳 **镜像管理**：预置镜像的查询和管理
...
- ✅ **多租户 Serverless 架构**：后端全面解耦单点配置，基于用户上下文会话动态实现 AK/SK 配置的鉴权与运行隔离
- ✅ **CORS 支持**：跨域请求支持
- ✅ **错误处理**：统一的错误处理机制
- ✅ **日志记录**：完整的日志记录系统
...
### 8. 开发机 (Dev Machine)
在线开发机实例管理：
- 查询分配的开发机列表以及资源详情（CPU、内存、加速卡与镜像等环境信息）
- 根据底层队列和资源池配额，申请搭建指定规格参数的专属开发节点
- 支持开发机状态干预操作（直接跳转 Web Terminal、随时关停及彻底回收实例）
...
### 10. 存储管理 (Storage)
底层存储资产统一管理：
- 将独立的 BOS Storage 和 PFS 实例并入统一台账进行注册与解绑
- 对接 AIHC 平台，实现数据集与物理存储资源的无缝联通

### 11. 数据仓库 (Data Warehouse)
数据仓库及 lakeFS 集成：
- 配置和接入本地/远端 lakeFS 数据源湖仓平台
- 对湖数据的多版本进行管理追踪

### 12. 计算资源 (Resource)
计算资源监控：
- 队列信息查询（支持高级参数过滤与按卡型统计）
- 资源池信息查询
- 加速卡使用情况
- CPU 和内存使用情况
...
### 存储管理
- `GET /api/storage/storages`：查询存储列表
- `GET /api/storage/storages/:id`：查询存储详情

### 数据仓库
- `GET /api/lakefs/repositories`：查询 lakeFS 代码库列表
- `POST /api/lakefs/repositories`：创建或配置数据仓库

### 开发机
- `POST /api/dev-instances`：查询分页开发机列表
- `GET /api/dev-instances/:id`：获取单台开发机详情
- `POST /api/dev-instances/create`：提交新建开发机申请
- `POST /api/dev-instances/:id/stop`：停止开发机
- `DELETE /api/dev-instances/:id`：彻底删除开发机
